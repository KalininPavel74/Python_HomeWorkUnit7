...
menu1 = '''Импортировать данные в Телефонный справочник:
1. start.csv
2. out.csv
:'''
...
# Выбрать и наполнить телефонный справочник первичными данными.
...
def looped_menu():
    ...
    while(True):
        ...
        if   a == 1: # 1. Отобразить первые 20 записей
            ...
            phone_number = view.input_str_value('Введите номера телефонов через запятую: ')
            phone_number = phone_number.replace(' ','').replace(',',export_import.CSV_SUB_SEPARATOR)
            lst = model.add_record([fio, phone_number])
            view.print_text('')
            if lst:
            ...
            phone_number = view.input_str_value('Введите номера телефонов через запятую: ')
            phone_number = phone_number.replace(' ','').replace(',',export_import.CSV_SUB_SEPARATOR)
            lst = model.update_by_id(id, [fio, phone_number])
            view.print_text('')
            if lst:
