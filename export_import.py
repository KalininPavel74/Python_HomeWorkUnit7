...
# записи в виде XML для Телефонного справочника
def export_DB_to_XML(file):
    ...
    for lst in list_of_lists:
        ...
        lines.append(f'{TAB_CHAR*2}<{model.FIELD_LAT_NAME_PNS}>')
        for s in lst[2].split(CSV_SUB_SEPARATOR):
            lines.append(f'{TAB_CHAR*3}<{model.FIELD_LAT_NAME_PN}>{s}</{model.FIELD_LAT_NAME_PN}>')
        lines.append(f'{TAB_CHAR*2}</{model.FIELD_LAT_NAME_PNS}>')
        lines.append(f'{TAB_CHAR}</{model.RECORD_LAT_NAME}>')
    lines.append(f'</{model.DB_LAT_NAME}>')
